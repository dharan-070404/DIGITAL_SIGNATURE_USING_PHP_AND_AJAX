...
$docno = substr($document, 0, 9);//echo $docno;
...
$fil = 'S';
$fullFilePath = 'http://' . $_SERVER["SERVER_NAME"] . '/web_auth/output/pdf/signed/' . $docno . $fil . '.pdf';

$pdfFile = fopen('output/pdf/signed/' . $docno . $fil . '.pdf', 'w');
fwrite($pdfFile, $decodedSignature);
fclose($pdfFile);

$folderPath = "output/pdf/signed/";
$file = $folderPath . $docno . $fil . '.pdf';
$file1=$docno.$fil.'.pdf';
if (filesize($file) > 10000) {
    $trimmedDocno = substr($docno, 0, -4);
    $fullFilePath1 = 'http://' . $_SERVER["SERVER_NAME"] . '/web_auth/output/pdf/signed/' . $docno . 'S.pdf';
} else {
    echo "Size ERROR";
}

$fullFilePath2 = 'http://' . $_SERVER["SERVER_NAME"] . '/web_auth/output/pdf/signed/' . $docno . $fil . '.pdf';
echo $fullFilePath2;
$sql = "update tbl_dept_incharge set $flag='".$file1."' where substr(dept,0,3)='".$did."' and $flag='Null'";
