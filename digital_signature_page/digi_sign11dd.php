...
// Create an array to store the documents to sign
var documentsToSign = [];

// Add your document filenames to the queue here
//documentsToSign.push('document1.pdf');
//documentsToSign.push('document2.pdf');
// Add more documents as needed

// Add your document filenames to the queue by fetching them from the server
$.ajax({
    url: 'get_document_list.php', // Replace with the actual URL of your PHP script
    dataType: 'json',
    success: function (data) {
        // Add the retrieved document filenames to the queue
        documentsToSign = documentsToSign.concat(data);

        // Start signing the documents
        startSigning();
    },
...
// Flag to track if signing is in progress
var signingInProgress = false;

// Function to initiate the signing process
function startSigning() {
    // Check if the signing process is already in progress
    if (signingInProgress) {
        return;
    }

    // Set the flag to indicate that signing is in progress
    signingInProgress = true;

    // Start signing the first document in the queue
    signNextDocument();
}

// Recursive function to sign the next document in the queue
function signNextDocument() {
    // Check if there are documents in the queue
    if (documentsToSign.length > 0) {
        // Get the next document to sign
        var documentName = documentsToSign.shift();
        // Call setData to sign the document
        setData("signData", documentName);
    } else {
        // All documents have been signed
        alert("All documents have been signed.");
        // Reset the flag to indicate that signing is complete
...
// Add the "Sign" button click event
...
<tr>
    ...
    //$tpt = 'D:/www/web_auth/output/pdf/unsigned/doctor'.$doc;
	?>
